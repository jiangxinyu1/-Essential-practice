                           ...
                           istream &is)
{
    ...
    while (is >> single_word)
    {
        ...
        {
            words[single_word]++;
            words_v.push_back(single_word);
        }
    }
    cout << "End of file reading ~" << endl;
    cout << "Complete data storage in vector!"<< endl;
    return &words_v;
}

void print_v (const vector<string> &words_v, ostream &os)
{
    vector<string>::const_iterator iter = words_v.begin();
    for( ; iter!=words_v.end(); ++iter)
    {
        os << *iter << '\t' ;
    }
    os << endl;
...
{
    sort(words_v.begin(), words_v.end(), Less_Than());
}